...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 23 11:20:12 2020
...
"""
Perform bandpass filtering to prep data for ERPs
...
import mne
import argparse
from config import (fname, erp_bandpass_fmin, erp_bandpass_fmax, n_jobs)
...
filt_raw =  raw.copy()
filt_raw.load_data().filter(l_freq=erp_bandpass_fmin, h_freq=erp_bandpass_fmax, l_trans_bandwidth='auto',
...
with mne.open_report(fname.report(subject='sub-'+ str(subj), session='ses-'+str(sess))) as report:
    report.add_figs_to_section(
        figs_before,
        captions='PSD before filtering- ERP',
        section='Sensor-level',
        replace=True
    )
    report.add_figs_to_section(
        figs_after,
        captions='PSD after filtering - ERP',
        section='Sensor-level',
        replace=True
    )
